---
title: Outlier Detection in River Network
...
- Developing statistical stools to detect anomalies in water-quality variables measured by in-situ sensors
...
```{r pringle-creek-map, echo=FALSE, out.height = "\\textheight",  out.width = "\\textwidth"}
knitr::include_graphics('plots/Pringle_creek_map.png')
```
...
```{r tsplots, echo=FALSE, out.width="\\textwidth"}
...
```{r turbplot, echo=FALSE, fig.width=8, fig.height=6}
...
knitr::include_graphics(here::here("plots",
                                   "classification_model_up_AR.pdf"))
...
load(here::here("plots", "confusion_mod_up_AR.rda"))

confusion_mod_up_AR %>%
  select(-NPV, -PPV) %>% 
  mutate(accuracy = round(accuracy, digits = 4),
         ER = round(ER, digits = 4),
         OP = round(OP, digits = 4)) %>%
  kable() %>%
...
load(here::here("plots", "tbl_comparison.rda"))
...
```{r echo=FALSE}
knitr::include_graphics(here::here("plots", "team-members",
                                   "Team_members.pdf"))

```

- Australian Research Council (ARC) Linkage project (grant number:
LP180101151) “Revolutionising high resolution water-quality monitoring in the information age”. 
